...
public final class CleartextHttp2ServerUpgradeHandler extends ByteToMessageDecoder {
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ChannelHandlerContext context = ctx.pipeline().context(httpServerCodec.getClass());
        ctx.pipeline()
                .addAfter(context.name(), null, httpServerUpgradeHandler);
    }
    ...
    private static io.netty.handler.codec.http2.CleartextHttp2ServerUpgradeHandler.PriorKnowledgeUpgradeEvent EVENT;

    private static io.netty.handler.codec.http2.CleartextHttp2ServerUpgradeHandler.PriorKnowledgeUpgradeEvent event() {
        if (EVENT == null) {
            try {
                Field field = io.netty.handler.codec.http2.CleartextHttp2ServerUpgradeHandler.PriorKnowledgeUpgradeEvent.class.getDeclaredField("INSTANCE");
                field.setAccessible(true);
                EVENT = (io.netty.handler.codec.http2.CleartextHttp2ServerUpgradeHandler.PriorKnowledgeUpgradeEvent) field.get(null);
