...
public class CompatibleHttpPostStandardRequestDecoder implements InterfaceHttpPostRequestDecoder {
     ...
     */
    @Override
    public CompatibleHttpPostStandardRequestDecoder offer(HttpContent content) {
        checkDestroyed();
    ...
    private static String decodeAttribute(String s, Charset charset) {
        try {
            return ServletUtil.decodeComponent(s, 0, s.length(), charset);
        } catch (IllegalArgumentException e) {
            return s;
//            throw new ErrorDataDecoderException("Bad string: '" + s + '\'', e);
    ...
    private static ByteBuf decodeAttribute(ByteBuf b, Charset charset) {
        int firstEscaped = b.forEachByte(urlEncodedDetector);
        if (firstEscaped == -1) {
...
//                String.format("Invalid hex byte at index '%d' in string: '%s'", idx, b.toString(charset)));
    ...
    /**
     * Destroy the and release all it resources. After this method
